...
function reloadCoach() {
  ...
  $.post(new_coach_url, {
    ...
    .done(function (data) {
      if (data["accepted"]) {
        $("#notification_div").html(
          $("#notification_div").html() +
            '<div class="alert alert-success alert-dismissible fade show m-2" role="alert" id="notif_' +
            id +
            '">\
        ...
        $("#notification_div").html(
          $("#notification_div").html() +
            '<div class="alert alert-danger alert-dismissible fade show m-2" role="alert" id="notif_' +
            id +
            "\">\
                    La requete n'a pas pu etre créée. Raison : " +
            data["reason"] +
            ".\
                </div>"
        );
    ...
    .fail(function (data) {
      $("#notification_div").html(
        $("#notification_div").html() +
          '<div class="alert alert-danger alert-dismissible fade show m-2" role="alert" id="notiffail_' +
          id +
          "\">\
